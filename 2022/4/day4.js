...
function answerPart2(data) {
  ...
  return elfPairs
    .map((pairSections) => (pairSections.length === _.unique(pairSections).length ? 0 : 1))
...
function answerPart1(data) {
  ...
  elfPairs.map((elfPair) => {
    const firstElfSectionContainedInSecondElfs =
      elfPair.firstElf.fromSection >= elfPair.secondElf.fromSection &&
      elfPair.firstElf.toSection <= elfPair.secondElf.toSection;
    const secondElfSectionContainedInFirstElfs =
      elfPair.secondElf.fromSection >= elfPair.firstElf.fromSection &&
      elfPair.secondElf.toSection <= elfPair.firstElf.toSection;
